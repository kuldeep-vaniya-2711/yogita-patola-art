...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            // CATEGORY SAFETY
            // --------------------------------------------------
            // If category is missing from the submitted form,
            // preserve the existing database category.
            // This prevents Mongoose "category is required".
            // --------------------------------------------------
    ...
    async (req, res) => {
        try {
            ...
            if (data.siteEmail) {
                ...
                if (!emailRegex.test(data.siteEmail)) {
                    deleteSettingsFiles(req.files);
            ...
            if (newHeroFile) {
                ...
                if (
                    oldHeroImage &&
                    oldHeroImage !== data.heroImage
                ) {
                    deleteImageFile(oldHeroImage);
                }
            } else {
                data.heroImage = clean(
                    heroImage ||
                    settings.heroImage
                );
            }
            ...
            if (newAboutFile) {
                ...
                if (
                    oldAboutImage &&
                    oldAboutImage !== data.aboutImage
                ) {
                    deleteImageFile(oldAboutImage);
                }
            } else {
                data.aboutImage = clean(
                    aboutImage ||
                    settings.aboutImage
                );
            }
            ...
            if (newFaviconFile) {
                ...
                if (
                    oldFavicon &&
                    oldFavicon !== data.faviconUrl
                ) {
                    deleteImageFile(oldFavicon);
                }
            } else {
                data.faviconUrl = clean(
                    faviconUrl ||
                    settings.faviconUrl
                );
            }
